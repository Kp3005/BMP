...
if(isset($_SESSION['lat']) && isset($_SESSION['lon'])){
	...
	$res=mysqli_query($con,"SELECT storetable.*, (
      3959 * acos (
      cos ( radians($lat) )
      * cos( radians( storetable.latitude ) )
      * cos( radians( storetable.longitude ) - radians($lon) )
      + sin ( radians($lat) )
      * sin( radians( storetable.latitude ) )
    )
) AS distance
FROM storetable
HAVING distance < 20
ORDER BY distance");
	...
	$res=mysqli_query($con,"select storetable.* from storetable");
	  ...
	  function success(pos){
		  ...
		  jQuery.ajax({
			  ...
			  success:function(result){
				  window.location.href='nearbyme.php'
			  }
                        ...
                        <div class="card-body">
                            ...
                            <hr>
                                    <?php while($row=mysqli_fetch_assoc($res)){
                                        ...
                                        ?>
                                            <div>
                                                <input type="checkbox" name="brands[]" value="<?= $row['id']; ?>" 
                                                    <?php if(in_array($row['id'], $checked)){ echo "checked"; } ?>
                                                 />
                                                <?= 
                                                // $dist = $row['distance'];
                                                // if($dist>20){
                                                  $row['store_name'],", ",$row['location'],", ",$row['city'];
                                                // }
                                                // else {
                                                    // echo $row['store_name'],", ",$row['location'],", ",$row['city'] , ", ", round($row['distance'],2), " km away";
